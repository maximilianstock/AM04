...
myModel<-eclust(whisky_tasting_notes, "kmeans", k = 3,nstart = 50, graph = FALSE)
...
# add it to the original data frame
whiskyTasting <- mutate(whisky_tasting_notes, cluster = as.factor(myModel$cluster))



#Use ggplot and results of your clustering algorithm as above
ggplot(whiskyTasting, aes(x = Medicinal, y = Sweetness, color =  as.factor(cluster))) +
  geom_jitter()+labs(color = "Cluster")
...
#First generate a new data frame with cluster centers and cluster numbers
myClusterCenters<-data.frame(cluster=as.factor(c(1:3)),myModel$centers)

#transpose this data frame
cluster_centers_t<-myClusterCenters %>% gather(variable,value,-cluster,factor_key = TRUE)
...
#Use the steps from above to visualize the separation of clusters with respect to first two components of principle components
fviz_cluster(myModel, whisky_tasting_notes, palette = "Set2", ggtheme = theme_minimal())
